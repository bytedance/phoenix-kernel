...
static bool stack_ovf_panic = false;

static void dump_backtrace_entry(unsigned long where, unsigned long depth, unsigned long fun_depth)
{
	if (stack_ovf_panic)
		printk(" %pS, depth=%lu/%lu\n", (void *)where, fun_depth, depth);
	else
		printk(" %pS\n", (void *)where);
}
...
void dump_backtrace(struct pt_regs *regs, struct task_struct *tsk)
{
	...
	unsigned long callee_fp;
	unsigned long callee_depth;
	...
	do {
		...
		/* the unwind frame might go across task stack and irq/exception stack, in which case
		 * (frame.fp-callee_fp) could be meaningless.
		 * */
		if((frame.fp > callee_fp) && (frame.fp <= (callee_fp + THREAD_SIZE))) {
			callee_depth = frame.fp - callee_fp;
		} else
			callee_depth = 0;

		/* skip until specified stack frame */
		if (!skip) {
			dump_backtrace_entry(frame.pc, THREAD_SIZE - (callee_fp & (THREAD_SIZE - 1)), callee_depth);
			 ...
			 * instead.
			 */
			dump_backtrace_entry(regs->pc, THREAD_SIZE - (callee_fp & (THREAD_SIZE - 1)), callee_depth);
		}
		callee_fp = frame.fp;
