...
/* Pico-Audio: Bringup analog headset {@ */
// used to record the uart gpio state before inserting the headet.
static int g_uart_gpio_init_state = 0;
static bool g_headset_inserted = false;

static int wcd_mbhc_get_uart_init_state()
{
	int rc = 0;
	/* ref schematic. usbc_uart_en_gpio high level disables the serial port */
	int usbc_uart_en_gpio = 1219; //1100~1279 corresponeds to AP 0~179

	g_uart_gpio_init_state = gpio_get_value(usbc_uart_en_gpio);
	pr_debug("%s: g_uart_gpio_init_state = %d\n", __func__, g_uart_gpio_init_state);
...
static int wcd_mbhc_usbc_ana_control_uart(struct wcd_mbhc *mbhc, bool active)
{
	...
	/* ref schematic. usbc_uart_en_gpio high level disables the serial port */
	int usbc_uart_en_gpio = 1219; //1100~1279 corresponeds to AP 0~179
	int uart_gpio_current_state = 0;

	pr_debug("%s: active = %d, g_headset_inserted = %d\n",
			__func__, active, g_headset_inserted);
	...
	uart_gpio_current_state = gpio_get_value(usbc_uart_en_gpio);
	pr_debug("%s: uart_gpio_current_state = %d, g_uart_gpio_init_state = %d\n",
		   __func__, uart_gpio_current_state, g_uart_gpio_init_state);
	if (active) {
		// disable uart when insert headset, need to set uart gpio to high level.
		if (0 == uart_gpio_current_state) {
			msm_cdc_pinctrl_select_active_state(usbc_uart_en_gpio_p);
		}
		g_uart_gpio_init_state = uart_gpio_current_state;
	} else {
		// resume uart gpio status after remove headset
		if (g_headset_inserted && (uart_gpio_current_state != g_uart_gpio_init_state)) {
			msm_cdc_pinctrl_select_sleep_state(usbc_uart_en_gpio_p);
		}
	}
	g_headset_inserted = active;
