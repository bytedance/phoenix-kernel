			...
			u8 *dev_num)
{
	...
	mutex_lock(&master->mlock);
	/*Modified by PICO_Driver jim.qiu for bug54370 : bring up audio for initializing wsa devices start*/
	{
		...
		for(;loop_cnt<3;loop_cnt++){
			...
			{
				pr_info("%s: get logical addr for device %llx successfully,ret=%d\n",
					__func__,dev_id,ret);
				break;
			}
		}
	}
	/*Modified by PICO_Driver jim.qiu for bug54370 : bring up audio for initializing wsa devices end*/
	mutex_unlock(&master->mlock);
	return ret;
}
