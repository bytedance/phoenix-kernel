...
static bool gpu_overclock_support;
#endif
...
#ifdef CONFIG_GPU_OVERCLOCK
		/*According to qcom/kona-v2-gpu.dtsi
		  speed_0(587M) and speed_1(670M) for adreno650*/
		if (adreno_dev->speed_bin == 0)  {
			adreno_dev->speed_bin = 1;
			gpu_overclock_support = true;
		}
#endif
		kfree(buf);
	}
...
#ifdef CONFIG_GPU_OVERCLOCK
	device->gpu_overclock_support = gpu_overclock_support;
	device->gpu_overclock_mode = false;
#endif
