...
static void adreno_dispatcher_issuecmds(struct adreno_device *adreno_dev)
{
	...
	if (!rt_mutex_trylock(&dispatcher->rt_mutex)) {
		_decrement_submit_now(device);
		goto done;
	}
	sched_save_priority_and_promote(&sched_attr);
	_adreno_dispatcher_issuecmds(adreno_dev);
	sched_restore_priority(&sched_attr);
	rt_mutex_unlock(&dispatcher->rt_mutex);
	_decrement_submit_now(device);
	return;
done:
...
static void adreno_dispatcher_work(struct kthread_work *work)
{
	...
	rt_mutex_lock(&dispatcher->rt_mutex);
...
int adreno_dispatcher_init(struct adreno_device *adreno_dev)
{
	...
	//mutex_init(&dispatcher->rt_mutex);
	__rt_mutex_init(&dispatcher->rt_mutex, NULL, NULL);
...
int adreno_dispatcher_idle(struct adreno_device *adreno_dev)
{
	 ...
	 */
	if (rt_mutex_is_locked(&dispatcher->rt_mutex) &&
		(rt_mutex_owner(&dispatcher->rt_mutex) == current))
		return -EDEADLK;
