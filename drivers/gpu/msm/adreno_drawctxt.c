...
static inline void _set_context_priority(struct adreno_context *drawctxt)
{
	...
	drawctxt->base.priority =
		...
		KGSL_CONTEXT_PRIORITY_SHIFT;
	/*if app call the sys node,set priority to max*/
	if (drawctxt->base.priority == KGSL_CONTEXT_PRIORITY_HIGH &&
		task_kgsl_ctx_priority_max(current))
		drawctxt->base.priority = 0;
}
