...
static ssize_t priority_level_store(struct device *dev,
                                struct device_attribute *attr,
                                 const char *buf, size_t count)
{
        int ret;
        ...
        if (ret)
                return ret;
        switch(level) {
                case KGSL_CONTEXT_PRIORITY_APP_MAX:
			task_set_kgsl_ctx_priority_max(current);
                        break;
                case KGSL_CONTEXT_PRIORITY_APP_HIGH:
			task_set_kgsl_ctx_priority_high(current);
                        break;
                default:
                        break;
        }
...
static DEVICE_ATTR_WO(priority_level);
