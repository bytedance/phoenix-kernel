...
#define ADRENO650_GPU_CLOCK 587000000
#endif
...
#ifdef CONFIG_GPU_OVERCLOCK
static ssize_t overclock_support_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	struct kgsl_device *device = dev_get_drvdata(dev);
	unsigned int support = device->gpu_overclock_support;

	return scnprintf(buf, PAGE_SIZE, "%d\n", support);
}

static ssize_t overclock_mode_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	struct kgsl_device *device = dev_get_drvdata(dev);
	unsigned int mode = device->gpu_overclock_mode;

	return scnprintf(buf, PAGE_SIZE, "%d\n", mode);
}

static ssize_t overclock_mode_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	struct kgsl_device *device = dev_get_drvdata(dev);
	...
	ret = kgsl_sysfs_store(buf, &mode);
	if (ret)
		return ret;

	mutex_lock(&device->mutex);
	device->gpu_overclock_mode = mode;

	if (device->gpu_overclock_mode == true)
		pwr->max_pwrlevel = 0;
	else
		pwr->max_pwrlevel = device->gpu_normal_pwrlevel;
...
static ssize_t normal_pwrlevel_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct kgsl_device *device = dev_get_drvdata(dev);
	unsigned int pwrlevel = device->gpu_normal_pwrlevel;
...
#ifdef CONFIG_GPU_OVERCLOCK
static DEVICE_ATTR_RO(overclock_support);
static DEVICE_ATTR_RW(overclock_mode);
static DEVICE_ATTR_RO(normal_pwrlevel);
#endif
...
#ifdef CONFIG_GPU_OVERCLOCK
	&dev_attr_overclock_support.attr,
	&dev_attr_overclock_mode.attr,
	&dev_attr_normal_pwrlevel.attr,
#endif
	NULL,
};
...
#ifdef CONFIG_GPU_OVERCLOCK
	device->gpu_overclock_mode = 0;
	device->gpu_normal_pwrlevel = 0;

	if (device->gpu_overclock_support) {
		for (i = 0; i < pwr->num_pwrlevels; i++) {
			if (pwr->pwrlevels[i].gpu_freq <= ADRENO650_GPU_CLOCK) {
				device->gpu_normal_pwrlevel = i;
				break;
			}
		}

		//printk("overclock found match freq=%d\r\n",device->gpu_normal_pwrlevel);
		pwr->max_pwrlevel = device->gpu_normal_pwrlevel;
	}
#endif
