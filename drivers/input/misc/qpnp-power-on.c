...
/* Pico-BSP: supported to trigger dump by long press combo key */
...
static ssize_t poff_reason_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	return sprintf(buf, "%s", pon->poff_reason);
}
static DEVICE_ATTR(poff_reason, 0444, poff_reason_show, NULL);

static ssize_t pon_reason_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);
	return sprintf(buf, "%s", pon->pon_reason);
}
static DEVICE_ATTR(pon_reason, 0444, pon_reason_show, NULL);
...
static int qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	//pr_warn("PMIC input: code=%d, status=0x%02X\n", cfg->key_code,
		//pon_rt_sts);
	...
	pr_warn("PMIC EV_KEY: code=%d, sts=0x%hhx, pon_type=%d\n",
...
static int qpnp_pon_config_init(struct qpnp_pon *pon,
				struct platform_device *pdev)
{
	/*Pico-BSP: supported to trigger dump by combo key*/
	u32 _pon_type = 0, is_combo_dump_enabled = 0;
	...
	/*Pico-BSP: supported to trigger dump by combo key*/
	is_combo_dump_enabled = get_force_dump_type();
	...
	for_each_available_child_of_node(pon->dev->of_node, cfg_node) {
		...
		/* Pico-BSP: supported to trigger dump by long press combo key */
		if ((!is_combo_dump_enabled) && (_pon_type == PON_KPDPWR_RESIN)) {
			dev_info(pon->dev, "Ignoring pon KPDPWR_AND_RESIN\n");
			pon->num_pon_config--;
			continue;
		}

		cfg = &pon->pon_cfg[i++];
		cfg->pon_type = _pon_type;
...
static int qpnp_pon_read_hardware_info(struct qpnp_pon *pon, bool sys_reset)
{
	...
	if (index >= ARRAY_SIZE(qpnp_pon_reason) || index < 0) {
		dev_info(dev, "PMIC@SID%d Power-on reason: Unknown and '%s' boot\n",
			 ...
			 cold_boot ? "cold" : "warm");
		sprintf(pon->pon_reason, "[-1] PMIC@SID%d Power-on reason: Unknown and '%s' boot\n", to_spmi_device(dev->parent)->usid, cold_boot ? "cold" : "warm");
	} else {
		...
		dev_info(dev, "PMIC@SID%d Power-on reason: %s and '%s' boot\n",
			 ...
			 cold_boot ? "cold" : "warm");
		sprintf(pon->pon_reason, "[%d] PMIC@SID%d Power-on reason: %s and '%s' boot\n", index, to_spmi_device(dev->parent)->usid, qpnp_pon_reason[index], cold_boot ? "cold" : "warm");
	}
	...
	if (index >= ARRAY_SIZE(qpnp_poff_reason) || index < 0 ||
		...
		dev_info(dev, "PMIC@SID%d: Unknown power-off reason\n",
			 to_spmi_device(dev->parent)->usid);
		sprintf(pon->poff_reason, "[-1] PMIC@SID%d: Unknown power-off reason\n",to_spmi_device(dev->parent)->usid);
	} else {
		pon->pon_power_off_reason = index;
		dev_info(dev, "PMIC@SID%d: Power-off reason: %s\n",
			 to_spmi_device(dev->parent)->usid,
			 qpnp_poff_reason[index]);
		sprintf(pon->poff_reason, "[%d] PMIC@SID%d: Power-off reason: %s\n",index, to_spmi_device(dev->parent)->usid, qpnp_poff_reason[index]);
	}
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	rc = device_create_file(dev, &dev_attr_poff_reason);
	if (rc) {
		dev_err(dev, "sysfs poff_reason file creation failed, rc=%d\n",
			rc);
		return rc;
	}

	rc = device_create_file(dev, &dev_attr_pon_reason);
	if (rc) {
		dev_err(dev, "sysfs pon_reason file creation failed, rc=%d\n",
			rc);
		return rc;
	}
...
static int qpnp_pon_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &dev_attr_poff_reason);
	device_remove_file(&pdev->dev, &dev_attr_pon_reason);
