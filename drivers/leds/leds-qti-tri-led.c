...
struct qpnp_led_dev {
	...
	bool			blinking;
	bool			breathing;
	/*Add by PICO rayson.zhang hunter.zhao for bug1488 bug54476 add led blink --start--*/
	unsigned long pico_blink;
	unsigned long pico_blink_on_ms;
	unsigned long pico_blink_off_ms;
	/*Add by PICO rayson.zhang hunter.zhao for bug1488 bug54476 add led blink --end--*/
};
...
/*Add by PICO rayson.zhang hunter.zhao for bug1488 bug54476 add led blink --start--*/
static ssize_t pico_blink_store(struct device *dev,
	...
	if (NULL != led) {
		...
		printk("pico_blink_store failed because of no data!\n");
		return -EINVAL;
	}
	if (blink > 0) {
		if (led->pico_blink_on_ms > 0 && led->pico_blink_off_ms > 0) {
			led->pico_blink = blink;
			led_blink_set(led_cdev,
				  &led->pico_blink_on_ms,
				  &led->pico_blink_off_ms);
		} else {
			qpnp_tri_led_set_brightness(led_cdev,LED_OFF);
		}
...
static ssize_t pico_blink_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	...
	if (NULL != led) {
		...
		printk("pico_blink_show failed because of no data!\n");
		return -EINVAL;
	}
}

static ssize_t pico_blink_on_ms_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count) {
	...
	led->pico_blink_on_ms = tmp;

	return count;
}

static ssize_t pico_blink_on_ms_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct qpnp_led_dev *led = NULL;
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	...
	if (NULL != led) {
		...
		printk("pico_blink_on_ms_show failed because of no data!\n");
		return -EINVAL;
	}
}

static ssize_t pico_blink_off_ms_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count) {
	...
	led->pico_blink_off_ms = tmp;

	return count;
}

static ssize_t pico_blink_off_ms_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	struct qpnp_led_dev *led = NULL;
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
...
static DEVICE_ATTR(pico_blink, 0664, pico_blink_show, pico_blink_store);
static DEVICE_ATTR(pico_blink_on_ms, 0664, pico_blink_on_ms_show, pico_blink_on_ms_store);
static DEVICE_ATTR(pico_blink_off_ms, 0664, pico_blink_off_ms_show, pico_blink_off_ms_store);

static struct attribute *pico_blink_attrs[] = {
	&dev_attr_pico_blink.attr,
	&dev_attr_pico_blink_on_ms.attr,
	&dev_attr_pico_blink_off_ms.attr,
	NULL
};
static const struct attribute_group pico_blink_attr_group = {
	.attrs = pico_blink_attrs,
};
/*Add by PICO rayson.zhang hunter.zhao for bug1488 bug54476 add led blink --end--*/
...
static int qpnp_tri_led_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < chip->num_leds; i++) {
		...
		/*Add by PICO rayson.zhang hunter.zhao for bug1488 bug54476 add led blink --start--*/
		sysfs_remove_group(&chip->leds[i].cdev.dev->kobj, &pico_blink_attr_group);
		/*Add by PICO rayson.zhang hunter.zhao for bug1488 bug54476 add led blink --end--*/
