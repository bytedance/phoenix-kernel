...
/* Added by gavin.zhao for bug 47081: read battery resistance -- start */
static int fg_battery_resistance = 0;
int fg_get_battery_resistance(struct fg_dev *fg, int *val)
{
	...
	*val = esr_uohms + rslow_uohms;
        ...
        fg_battery_resistance = *val;
        pr_debug("fg_get_battery_resistance uohms =%d\n", fg_battery_resistance);
	return 0;
}
/* Added by gavin.zhao for bug 47081: read battery resistance -- end */
...
/* Added by gavin.zhao for bug 47081: read battery resistance -- start */
static int fg_status_write(void *data, u64 val)
{
        return 0;
}
static int fg_status_read(void *data, u64 *val)
{
        *val = fg_battery_resistance;
        return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(fg_status_ops,fg_status_read,fg_status_write, "%02lld\n");
/* Added by gavin.zhao for bug 47081: read battery resistance -- end */
...
int fg_debugfs_create(struct fg_dev *fg)
{
	int rc;
	struct dentry *file;
        /* Added by gavin.zhao for bug 42629: DC_IN detect -- start */
        struct dentry *dfs_dc_detect;
       // struct dentry *file_dc_detect;
        struct dentry *file_fg_detect;
        /* Added by gavin.zhao for bug 42629: DC_IN detect -- end */
	...
	if (IS_ERR_OR_NULL(fg->dfs_root)) {
        ...
        /* Added by gavin.zhao for bug 47081: read battery resistance -- start */
        dfs_dc_detect = debugfs_create_dir("dc_detect", fg->dfs_root);
        if (!dfs_dc_detect) {
                pr_err("error creating dc detect dfs rc=%ld\n",
                       (long)dfs_dc_detect);
                return -ENOMEM;
        }

        // cat /sys/kernel/debug/fg/dc_detect/fg_status
        file_fg_detect = debugfs_create_file("fg_status",
                                S_IFREG | S_IWUSR | S_IRUGO | S_IWUGO,
                                dfs_dc_detect, fg,
                                &fg_status_ops);
        if (IS_ERR_OR_NULL(file_fg_detect))
                pr_err("Couldn't create fg status file rc=%ld\n",
                        (long)file_fg_detect);
        /* Added by gavin.zhao for bug 47081: read battery resistance -- end */
