				    ...
				    const union power_supply_propval *val)
{
	...
	power_supply_get_property(chg->usb_psy,
			POWER_SUPPLY_PROP_PD_VOLTAGE_MAX, &pval);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read pd voltage rc=%d\n", rc);
		return rc;
	}

	/* If the PD voltage is not at vSafe5V, need to limit
	 * the max current at 2.23A for 20W power charging. */
	pd_icl = (pval.intval == 5000000) ?
			val->intval : min(val->intval, 2230000);

	if (chg->pd_active) {
		icl = get_client_vote(chg->usb_icl_votable, PD_VOTER);
		rc = vote(chg->usb_icl_votable, PD_VOTER, true, pd_icl);
		if (pd_icl != icl) {
			power_supply_changed(chg->usb_psy);
			pr_info("usbpd: set max current=%dmA at voltage=%dmV\n",
					pd_icl / 1000, pval.intval / 1000);
		}
	} else {
		rc = -EPERM;
	}
