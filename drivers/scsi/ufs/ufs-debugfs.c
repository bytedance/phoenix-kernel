...
static int ufsdbg_dump_device_health_show(struct seq_file *file, void *data)
{
	...
	ufs_pre_eol = desc_buf[HEALTH_DESC_PARAM_EOL_INFO];
	ufs_lifetime_est_a = desc_buf[HEALTH_DESC_PARAM_LIFE_TIME_EST_A];
	ufs_lifetime_est_b = desc_buf[HEALTH_DESC_PARAM_LIFE_TIME_EST_B];

	pr_debug("ufs_pre_eol=%02X, ufs_lifetime_est_a=%02X, ufs_lifetime_est_b=%02X\n",
		ufs_pre_eol, ufs_lifetime_est_a, ufs_lifetime_est_b);

	for (i = 0; i < 32; i++) {
		sprintf(&str_prop[i*3], "%02X ", desc_buf[HEALTH_DESC_PARAM_VENDOR_PROP+i]);
	}
