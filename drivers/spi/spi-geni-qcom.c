...
/*Add by PICO_Driver clark.tian for task55837: Ble-Mcu-52840 spi debug --start--*/
#define BLE_LOAD_MAX_UA   150000
struct regulator *reg = NULL;
struct mcu_power_data{
	struct mutex	state_lock;
	int count;
	int bu_count;
};
struct mcu_power_data 	mcudev;
/*Add by PICO_Driver clark.tian for task55837: Ble-Mcu-52840 spi debug --end--*/
struct gsi_desc_cb {
...
/*Add by PICO_Driver clark.tian for task55837: Ble-Mcu-52840 spi debug --start--*/
int spidev_mcu_power_off(void)
{
	int ret=0;
	printk("[%s]\n",__func__);
	mutex_lock(&mcudev.state_lock);
	if(mcudev.count > 0){
		ret = regulator_disable(reg);

		if (ret) {
			printk("[%s] unable to trun off ldo!\n",__func__);
			ret = -EPERM;
		}
		mcudev.count--;
		printk("[%s] count=%d\n",__func__,mcudev.count);
	}
	mutex_unlock(&mcudev.state_lock);
	printk("[%s] ret=%d\n",__func__,ret);
	return ret;
}

int spidev_mcu_power_on(void)
{
	int ret = 0;
	printk("[%s]\n",__func__);
	mutex_lock(&mcudev.state_lock);
	...
	if (ret) {
		printk("[%s] unable to enable the vdd!\n",__func__);
		ret = -EPERM;
	}
	mcudev.count++;
	printk("[%s] count=%d\n",__func__,mcudev.count);
...
EXPORT_SYMBOL_GPL(spidev_mcu_power_on);
EXPORT_SYMBOL_GPL(spidev_mcu_power_off);
EXPORT_SYMBOL_GPL(spidev_mcu_show_counter);
static int enable_mcu_power_write(void *data, u64 val)
{
	 if(val > 0)
		spidev_mcu_power_on();
	 return mcudev.count;
}
static int enable_mcu_power_read(void *data, u64 *val)
{
        *val = mcudev.count;
        return 0;
}
DEFINE_SIMPLE_ATTRIBUTE(enable_mcu_power_ops, enable_mcu_power_read, enable_mcu_power_write, "0x%02llx\n");
static void spi_powerfs_init(struct platform_device *pdev)
{
        struct dentry *spi_root;
        struct dentry *file;

	spi_root= debugfs_create_dir("mcu52840", NULL);
        file = debugfs_create_file("mcu_power",
                                S_IFREG | S_IWUSR | S_IRUGO | S_IWUGO,
                                spi_root, pdev, &enable_mcu_power_ops);
        if (IS_ERR_OR_NULL(file))
                pr_err("Couldn't spi power file rc=%ld\n",(long)file);

}

static void spi_power_init(struct platform_device *pdev)
{
	char   *default_name = "894000.spi";
	int ret = 0;
	if(strcmp(default_name,pdev->name) == 0){
		mutex_init(&mcudev.state_lock);
		spi_powerfs_init(pdev);
		mcudev.count = 0;
		mcudev.bu_count= 0;
		...
		ret = regulator_set_load(reg, BLE_LOAD_MAX_UA);
		if (ret) {
...
/*Add by PICO_Driver clark.tian for task42623: Ble-Mcu-52840 spi debug --end--*/
...
exit_rt_suspend:
	/*Modify by PICO_Driver clark.tian for task60764: Ble-Mcu-52840 spi debug --start--*/
	ret = fake_se_geni_resources_off(&geni_mas->spi_rsc);
...
{
	int ret = 0;
	/*Add by PICO_Driver clark.tian for task60764: Ble-Mcu-52840 spi debug --start--*/
	struct spi_master *pvr_spi = get_spi_master(dev);
	struct spi_geni_master *pvr_geni_mas = spi_master_get_devdata(pvr_spi);
	struct se_geni_rsc *pvr_rsc;
	pvr_rsc = &pvr_geni_mas->spi_rsc;
	/*Add by PICO_Driver clark.tian for task60764: Ble-Mcu-52840 spi debug --end--*/
	...
	/*Add by PICO_Driver clark.tian for task60764: Ble-Mcu-52840 spi debug --start--*/
	pinctrl_select_state(pvr_rsc->geni_pinctrl, pvr_rsc->geni_gpio_sleep);
	/*Add by PICO_Driver clark.tian for task60764: Ble-Mcu-52840 spi debug --end--*/
	return ret;
}
#else
