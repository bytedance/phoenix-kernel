...
#define GDMCU_LOAD_MAX_UA   150000
static int gd_mcu_boot;
static int gd_mcu_reset;
static int gd_mcu_sync;
						...
						unsigned long arg)
{
	...
	case GDMCU_SET_POWER_STATE: {
	        ret = get_user(tmp, (__u32 __user *)arg);
                if(ret == 0){
		   if (tmp){
			ret = regulator_enable(reg);
			if (ret) {
				dev_err(uport->dev,"unable to enable gd mcu vdd\n");
				ret = -EPERM;
			}
			dev_info(uport->dev,"enable gd mcu vdd\n");
			}else{
			    ret = regulator_disable(reg);
			    if (ret) {
				dev_err(uport->dev,"unable to disable gd mcu vdd\n");
				ret = -EPERM;
			    }
			    dev_info(uport->dev,"disable gd mcu vdd\n");
			}
		}
		break;
	}
	case GDMCU_SET_BOOT_GPIO_STATE: {
		ret = get_user(tmp, (__u32 __user *)arg);
		if(ret == 0){
			if(tmp){
				gpio_set_value(gd_mcu_boot, 1);//SET GPIO HIGH
				printk("[%s] Set GDMCU boot gpio High\n",__func__);
			}else{
				gpio_set_value(gd_mcu_boot, 0);//SET GPIO LOW
				printk("[%s] Set GDMCU boot gpio Low\n",__func__);
			}
		}
		break;
	}
	case GDMCU_SET_RESET_GPIO_STATE: {
		ret = get_user(tmp, (__u32 __user *)arg);
		if(ret == 0){
			if(tmp){
				gpio_set_value(gd_mcu_reset, 1);//SET GPIO HIGH
				dev_info(uport->dev,"Set GDMCU reset gpio High\n");
			}else{
				gpio_set_value(gd_mcu_reset, 0);//SET GPIO LOW
				dev_info(uport->dev,"Set GDMCU reset gpio Low\n");
			}
		}
		break;
	}
	case GDMCU_SET_SYNC_GPIO_STATE: {
		ret = get_user(tmp, (__u32 __user *)arg);
		if(ret == 0){
			if(tmp){
				gpio_set_value(gd_mcu_sync, 1);//SET GPIO HIGH
				dev_info(uport->dev,"Set GDMCU sync gpio High\n",__func__);
			}else{
				gpio_set_value(gd_mcu_sync, 0);//SET GPIO LOW
				dev_info(uport->dev,"Set GDMCU sync gpio Low\n",__func__);
			}
		}
		break;
	}
...
static void msm_geni_serial_gdmcu_power_init(struct platform_device *pdev)
{
	char   *default_name = "984000.qcom,qup_uart";
	int ret = 0;
	if(strcmp(default_name,pdev->name) == 0){
		reg = devm_regulator_get(&pdev->dev, "vdd");
		if (IS_ERR(reg)) {
			dev_err(&pdev->dev,"unable to get vdd supply\n");
		}
		ret = regulator_set_load(reg, GDMCU_LOAD_MAX_UA);
		if (ret) {
			dev_err(&pdev->dev,"vdd regulator set_load failed ret=%d", ret);
			regulator_put(reg);
		}
		ret = regulator_enable(reg);
		if (ret) {
			dev_err(&pdev->dev,"unable to enable gd mcu vdd\n");
		}
	}
}
...
static int msm_geni_serial_probe(struct platform_device *pdev)
{
	...
	u32 wake_char = 0;
        char   *gd_uart_name = "984000.qcom,qup_uart";
...
//PicoBSP: -->end
	...
	dev_info(&pdev->dev, "dev_port->name=%s\n",dev_port->name);
	dev_info(&pdev->dev, "pdev->name=%s\n",pdev->name);
	...
	if (is_console) {
		...
		if (strcmp(gd_uart_name,pdev->name) == 0){
                    msm_geni_serial_gdmcu_power_init(pdev);
                    if ((gd_mcu_boot = of_get_named_gpio(pdev->dev.of_node, "boot-gpio", 0)) < 0){
                        dev_err(&pdev->dev, "gd mcu get boot-gpio error!\n");
                        goto exit_geni_serial_probe;
                    }
                    if ((gd_mcu_reset = of_get_named_gpio(pdev->dev.of_node, "reset-gpio", 0)) < 0){
                        dev_err(&pdev->dev, "gd mcu get reset-gpio error!\n");
                        goto exit_geni_serial_probe;
                    }
                    if ((gd_mcu_sync = of_get_named_gpio(pdev->dev.of_node, "sync-gpio", 0)) < 0){
                        dev_err(&pdev->dev, "gd mcu get int-gpio error!\n");
                        goto exit_geni_serial_probe;
                    }
                    if (gpio_is_valid(gd_mcu_boot)) {
                        ret = gpio_request(gd_mcu_boot, "boot-gpio");
                        if(ret < 0){
                            dev_err(&pdev->dev, "gd mcu get boot-gpio request error!\n");
                        }
                        gpio_direction_output(gd_mcu_boot, 1);
                        gpio_set_value(gd_mcu_boot, 1);
                    }

                    if (gpio_is_valid(gd_mcu_reset)) {
                        ret = gpio_request(gd_mcu_reset, "reset-gpio");
                        if(ret < 0){
                                dev_err(&pdev->dev, "gd mcu get reset-gpio request error!\n");
                                gpio_free(gd_mcu_reset);
                        }
                        gpio_direction_output(gd_mcu_reset, 0);
                        gpio_set_value(gd_mcu_reset, 0);
                    }
                    if (gpio_is_valid(gd_mcu_sync)) {
                        ret = gpio_request(gd_mcu_sync, "sync-gpio");
                        if(ret < 0){
                            dev_err(&pdev->dev, "gd mcu get sync-gpio request error!\n");
                            gpio_free(gd_mcu_sync);
                        }
                        gpio_direction_output(gd_mcu_sync, 0);
                        gpio_set_value(gd_mcu_sync, 0);
                    }
		}
	}
...
static int msm_geni_serial_sys_suspend_noirq(struct device *dev)
{
	...
	if (strcmp(gd_uart_name,pdev->name) == 0){
            ...
            if (ret) {
                ...
                dev_info(uport->dev,"gd mcu power off\n");
            }
            gpio_set_value(gd_mcu_boot, 0);
            gpio_set_value(gd_mcu_reset, 0);
            gpio_set_value(gd_mcu_sync, 0);
	}
	return 0;
	...
	if (strcmp(gd_uart_name,pdev->name) == 0){
	     gpio_set_value(gd_mcu_boot, 1);
            ...
            if (ret) {
                ...
                dev_info(uport->dev,"gd mcu power on\n");
            }
	}
	return 0;
}
