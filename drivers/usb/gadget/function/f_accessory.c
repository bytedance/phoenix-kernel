...
#define BULK_MAX_BUFFER_SIZE    1048576
...
static unsigned int acc_tx_rx_req_len = BULK_MAX_BUFFER_SIZE;
module_param(acc_tx_rx_req_len, uint, 0644);

static int acc_tx_reqs = TX_REQ_MAX;
module_param(acc_tx_reqs, uint, 0644);
				...
				struct usb_endpoint_descriptor *out_desc)
{
	...
	if (acc_tx_rx_req_len % 1024)
		acc_tx_rx_req_len = (acc_tx_rx_req_len - (acc_tx_rx_req_len % 1024));
	/* now allocate requests for our endpoints */
retry_tx_alloc:
	for (i = 0; i < acc_tx_reqs; i++) {
		req = acc_request_new(dev->ep_in, acc_tx_rx_req_len);
		if (!req) {
			if (acc_tx_rx_req_len <= BULK_BUFFER_SIZE)
				goto fail;
			while ((req = req_get(dev, &dev->tx_idle)))
				acc_request_free(req, dev->ep_in);
			acc_tx_rx_req_len = BULK_BUFFER_SIZE;
			acc_tx_reqs = TX_REQ_MAX;
			pr_err("acc tx req alloc failed");
			goto retry_tx_alloc;
		}
...
retry_rx_alloc:
	for (i = 0; i < RX_REQ_MAX; i++) {
		req = acc_request_new(dev->ep_out, acc_tx_rx_req_len);
		if (!req) {
			if (acc_tx_rx_req_len <= BULK_BUFFER_SIZE)
				goto fail;
			for (--i; i >= 0; i--)
			...
			acc_tx_rx_req_len = BULK_BUFFER_SIZE;
			pr_err("acc rx req alloc failed");
			goto retry_rx_alloc;
		}
	...
	size_t count, loff_t *pos)
{
	...
	if (dev->disconnected) {
        ...
        if (count > acc_tx_rx_req_len)
            count = acc_tx_rx_req_len;
