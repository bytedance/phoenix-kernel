...
static int msm_ssphy_qmp_init(struct usb_phy *uphy)
{
	...
	if (phy->txmgn) {
		writel_relaxed((u32)phy->txmgn,
			       phy->base + 0x088E9D38 - 0x88e8000);
	}

	if (phy->txdeemph) {
		writel_relaxed((u32)phy->txdeemph,
			       phy->base + 0x088E9D6C - 0x88e8000);
	}

	if (phy->a_txdrv) {
		writel_relaxed((u32)phy->a_txdrv,
			       phy->base + 0x088E9214 - 0x88e8000);
	}

	if (phy->b_txdrv) {
		writel_relaxed((u32)phy->b_txdrv,
			       phy->base + 0x088E9614 - 0x88e8000);
	}

	if (phy->a_pre) {
		writel_relaxed((u32)phy->a_pre,
			       phy->base + 0x088E9308 - 0x88e8000);
	}

	if (phy->b_pre) {
		writel_relaxed((u32)phy->b_pre,
			       phy->base + 0x088E9708 - 0x88e8000);
	}

	if (phy->a_post) {
		writel_relaxed((u32)phy->a_post,
			       phy->base + 0x088E920C - 0x88e8000);
	}

	if (phy->b_post) {
		writel_relaxed((u32)phy->b_post,
			       phy->base + 0x088E960C - 0x88e8000);
	}
...
static u32 address = 0x88e8000;
static int address_get(void *data, u64 *val)
{
	*val = (u64)address;
	return 0;
}

static int address_set(void *data, u64 val)
{
	address = (u32)val;
	return 0;
}
...
{
	struct msm_ssphy_qmp *phy = (struct msm_ssphy_qmp *)data;
	*val = (u64)readl_relaxed(phy->base + address - 0x88e8000);
	return 0;
}
...
static int data_set(void *data, u64 val)
{
	...
	writel_relaxed((u32)val,
		       phy->base + address - 0x88e8000);
	return 0;
}

DEFINE_DEBUGFS_ATTRIBUTE(address_fops,
			 address_get,
			 address_set, "0x%x\n");

DEFINE_DEBUGFS_ATTRIBUTE(data_fops,
			 data_get,
			 data_set, "0x%x\n");
...
static void msm_ssphy_create_debugfs(struct msm_ssphy_qmp *phy)
{
	...
	debugfs_create_file("address", 0666, phy->root, phy,
			    &address_fops);
	debugfs_create_file("data", 0666, phy->root, phy,
			    &data_fops);
	debugfs_create_x8("txmgn", 0644, phy->root, &phy->txmgn);
	debugfs_create_x8("txdeemph", 0644, phy->root, &phy->txdeemph);
	debugfs_create_x8("a_txdrv", 0644, phy->root, &phy->a_txdrv);
	debugfs_create_x8("b_txdrv", 0644, phy->root, &phy->b_txdrv);
	debugfs_create_x8("a_pre", 0644, phy->root, &phy->a_pre);
	debugfs_create_x8("b_pre", 0644, phy->root, &phy->b_pre);
	debugfs_create_x8("a_post", 0644, phy->root, &phy->a_post);
	debugfs_create_x8("b_post", 0644, phy->root, &phy->b_post);
}
