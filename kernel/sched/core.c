...
int sched_fork(unsigned long clone_flags, struct task_struct *p)
{
	...
	//Smt:[FEAT_UI_PRIORITY] {@
#ifdef CONFIG_UI_FIRST
	p->ui_priority = 0;
#endif
	//@}
			       ...
			       const struct sched_param *param, bool check)
{
	...
	//Smt:[FEAT_UI_PRIORITY] {@
#ifdef CONFIG_UI_FIRST
	if (param->sched_priority > 0xFFFF) {
		int16_t val = param->sched_priority >> 16;

		if (val == 0xFF)
			p->ui_priority = 0;
		else
			p->ui_priority = val & 0xFF;
		return 0;
	}
#endif
...
int sched_save_priority_and_promote(struct sched_attr *attr)
{
	struct sched_attr new_attr;
	int retval;

	attr->size = SCHED_ATTR_SIZE_VER0;
	attr->sched_policy = current->policy;
	if (task_has_dl_policy(current))
		__getparam_dl(current, attr);
	else if (task_has_rt_policy(current))
		attr->sched_priority = current->rt_priority;
	else
		attr->sched_nice = task_nice(current);

	memcpy(&new_attr, attr, sizeof(struct sched_attr));
	new_attr.sched_policy=SCHED_FIFO;
	new_attr.sched_priority=40;
	new_attr.sched_nice=MAX_NICE;
	retval = sched_setattr_nocheck(current, &new_attr);
	return retval;
}

int sched_restore_priority(struct sched_attr *attr)
{
	int retval;
	retval = sched_setattr_nocheck(current, attr);
	return retval;
}
...
//Smt:[FEAT_UI_PRIORITY] {@
#if CONFIG_UI_FIRST
int binder_set_cpus(struct task_struct *p,
			   const struct cpumask *new_mask, bool check)
{
	const struct cpumask *cpu_valid_mask = cpu_active_mask;
	unsigned int dest_cpu;
	...
	if (check && cpumask_equal(&p->cpus_allowed, new_mask)) {
		goto out;
	}
