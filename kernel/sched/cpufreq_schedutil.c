				  ...
				  unsigned long util, unsigned long max)
{
	...
	switch(sg_policy->tunables->up_freq_level){
		case 0:
			freq = map_util_freq(util, freq, max);
			break;
		case 1:
...
static ssize_t up_freq_level_show(struct gov_attr_set *attr_set, char *buf)
{
	struct sugov_tunables *tunables = to_sugov_tunables(attr_set);

	return scnprintf(buf, PAGE_SIZE, "%u\n", tunables->up_freq_level);
}
...
static ssize_t up_freq_level_store(struct gov_attr_set *attr_set,
					const char *buf, size_t count)
{
	struct sugov_tunables *tunables = to_sugov_tunables(attr_set);
	unsigned int level;
...
static struct governor_attr up_freq_level = __ATTR_RW(up_freq_level);
		...
		struct sugov_tunables *tunables)
{
	...
	cached->up_freq_level = tunables->up_freq_level;
}
...
static void sugov_tunables_restore(struct cpufreq_policy *policy)
{
	...
	tunables->up_freq_level = cached->up_freq_level;
}
