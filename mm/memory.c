...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	...
	if (SWAP_IS_UFS(si))
		dec_mm_counter_fast(vma->vm_mm, MM_SWAPENTS_UFS);
...
bool do_swap_page_bytedance(struct vm_fault *vmf)
{
	struct vm_area_struct *vma = vmf->vma;
	struct page *page = NULL, *swapcache;
	struct mem_cgroup *memcg;
	...
	int exclusive = 0;
	vm_fault_t ret;
	bool unsuitable_entry = false;

	/* ByteDance-BSP: this might be not necessary as of arm64 it is empty
	 * function and always return 0 */
	ret = pte_unmap_same(vmf);
	if (ret) {
		/*
		 * If pte != orig_pte, this means another thread did the
	...
	if (unlikely(non_swap_entry(entry))) {
		...
		/* ByteDance-BSP: not quite sure if code could reach here so
		 * make sure of this case */
		unsuitable_entry = true;
		goto out;
	}
	 ...
	 */
	si = swp_swap_info(entry);
	if (!swapcount_suitable_to_pushswap_from_zram(si, entry)) {
		unsuitable_entry = true;
		goto out;
	}
	if (si->flags & SWP_SYNCHRONOUS_IO && __swap_count(si, entry) == 1)
	...
	if (unlikely(page != swapcache && swapcache)) {
		...
		do_page_add_anon_rmap(page, vma, vmf->address, exclusive);
		mem_cgroup_commit_charge(page, memcg, true, false);
		/* ByteDance-BSP: comment out to not activate it so when
		 * we fail to pushswap then it could be reclaimed sooner.
		 * activate_page(page);
		 */
	}

	swap_free(entry);
	/* ByteDance-BSP:  always free zram swap slot cause it is to be stored into ufs swap*/
	if (true || mem_cgroup_swap_full(page) ||
	    (vmf->vma_flags & VM_LOCKED) || PageMlocked(page))
		try_to_free_swap(page);
	unlock_page(page);
	if (page != swapcache && swapcache) {
