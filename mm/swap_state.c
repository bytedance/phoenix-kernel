 ...
 */
int add_to_swap(struct page *page, bool pushswap)
{
	swp_entry_t entry;
	int err;
	VM_BUG_ON_PAGE(!PageLocked(page), page);
	...
	entry = get_swap_page(page, pushswap);
	if (!entry.val)
		return 0;
			...
			struct vma_swap_readahead *ra_info)
{
	...
	/* ByteDance-BSP: since the forward page can be in ufsswap already, so ignore readahead*/
	if ((max_win == 1) || vmf->flags & FAULT_FLAG_RECLAIM_TO_STORAGE) {
		ra_info->win = 1;
		return;
	}
