...
int add_to_swap_wrapper(struct page *page, struct scan_control *sc)
{
	if (sc->target_vma && test_bit(MMF_RECLAIM_TO_STORAGE_START, &sc->target_vma->vm_mm->flags))
		return add_to_swap(page, true);
	else
		return add_to_swap(page, false);
}
	...
	while (!list_empty(page_list)) {
		...
		if (PageAnon(page) && PageSwapBacked(page)) {
			if (!PageSwapCache(page)) {
				...
				if (!add_to_swap_wrapper(page, sc)) {
					if (!PageTransHuge(page))
						goto activate_locked;
