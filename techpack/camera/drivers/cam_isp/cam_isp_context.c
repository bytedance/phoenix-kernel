	...
	struct cam_isp_context *ctx_isp, void *evt_data)
{
	...
	if (req_isp->bubble_report && ctx->ctx_crm_intf &&
		...
		CAM_WARN(CAM_ISP,
			...
			ctx->ctx_id);
		ctx->ctx_crm_intf->notify_err(&notify);
		atomic_set(&ctx_isp->process_bubble, 1);/*Modify by PICO_Driver sophia.wang for bug58509:merge qcom patch for bubble problem in cv3*/
	} else {
		req_isp->bubble_report = 0;
	}
...
/*Modify by PICO_Driver sophia.wang for bug58509:merge qcom patch for bubble problem in cv3 start*/
	...
	struct cam_isp_context *ctx_isp, void *evt_data)
{
	...
	ctx_isp->sof_timestamp_val = sof_event_data->timestamp;
	ctx_isp->boot_timestamp = sof_event_data->boot_time;
	CAM_INFO(CAM_ISP, "ctx id: %d frame id: %lld time stamp:0x%llx",
	...
	while (!list_empty(&ctx->active_req_list)) {
		...
		if (req_isp->bubble_detected) {
			...
			CAM_INFO(CAM_ISP, "ctx id: %d Delayed bufdone for req: %lld",
...
/*Modify by PICO_Driver sophia.wang for bug58509:merge qcom patch for bubble problem in cv3 end*/
	...
	struct cam_isp_context *ctx_isp, void *evt_data)
{
	...
	CAM_INFO(CAM_ISP, "ctx id %d next Substate[%s]",ctx->ctx_id,
		__cam_isp_ctx_substate_val_to_type(
		ctx_isp->substate_activated));
...
static struct cam_isp_ctx_irq_ops
	...
	/* APPLIED */
	{
		.irq_ops = {
			...
			__cam_isp_ctx_rdi_only_reg_upd_in_applied_state,/*Modify by PICO_Driver sophia.wang for bug58509:merge qcom patch for bubble problem in cv3*/
			NULL,
			NULL,
			__cam_isp_ctx_buf_done_in_applied,
		},
