...
static int dp_parser_gpio(struct dp_parser *parser)
{
	...
	static const char * const dp_gpios_v0[] = {
		"qcom,aux-en-gpio",
		"qcom,aux-sel-gpio",
       ...
       static const char * const dp_gpios_v1[] = {
		"qcom,usbplug-cc-gpio",
		"qcom,uart-en-gpio",
	};
	...
	if (of_find_property(of_node, "qcom,dp-hpd-gpio", NULL)) {
        ...
        if (hw_version==0)
        {
          ...
          DP_INFO("%s hw_version=%d,parser->gpio_aux_switch=%d!\n", __func__,hw_version,parser->gpio_aux_switch);
	  mp->gpio_config = devm_kzalloc(dev,
		sizeof(struct dss_gpio) * ARRAY_SIZE(dp_gpios_v0), GFP_KERNEL);
	if (!mp->gpio_config)
		return -ENOMEM;

	mp->num_gpio = ARRAY_SIZE(dp_gpios_v0);

	for (i = 0; i < ARRAY_SIZE(dp_gpios_v0); i++)
           {
		...
		strlcpy(mp->gpio_config[i].gpio_name, dp_gpios_v0[i],
       ...
       else
      {
        ...
        DP_INFO("%s hw_version=%d,parser->gpio_aux_switch=%d!\n", __func__,hw_version,parser->gpio_aux_switch);
	mp->gpio_config = devm_kzalloc(dev,
		sizeof(struct dss_gpio) * ARRAY_SIZE(dp_gpios_v1), GFP_KERNEL);
	if (!mp->gpio_config)
		return -ENOMEM;

	mp->num_gpio = ARRAY_SIZE(dp_gpios_v1);

	for (i = 0; i < ARRAY_SIZE(dp_gpios_v1); i++)
        {
		mp->gpio_config[i].gpio = of_get_named_gpio(of_node,
			dp_gpios_v1[i], 0);
		...
		strlcpy(mp->gpio_config[i].gpio_name, dp_gpios_v1[i],
			sizeof(mp->gpio_config[i].gpio_name));
		mp->gpio_config[i].value = 0;
	}
      }
