...
{
	int rc = 0, i,array_size;
        int hw_version=0;
	struct device *dev;
	struct dss_module_power *mp;
	static const char * const gpio_names_v0[] = {
		"aux_enable", "aux_sel", "usbplug_cc",
	};

	static const char * const gpio_names_v1[] = {
		 "usbplug_cc",
	};
	...
	mp = &power->parser->mp[DP_CORE_PM];
        ...
        if (!hw_version)
           array_size=ARRAY_SIZE(gpio_names_v0);
        else
           array_size=ARRAY_SIZE(gpio_names_v1);
	...
	for (i = 0; i < array_size; i++) {
		...
		if (gpio_is_valid(gpio)) {
                          if (!hw_version)
                          {
		            rc = devm_gpio_request(dev, gpio, gpio_names_v0[i]);
			    if (rc) {
				...
				goto error;
                         ...
                         {
                            rc = devm_gpio_request(dev, gpio, gpio_names_v1[i]);
			    if (rc) {
				DP_ERR("request %s gpio failed, rc=%d\n",
					       gpio_names_v1[i], rc);
				goto error;
		...
		if (gpio_is_valid(config->gpio)) {
			...
			if (dp_power_find_gpio(config->gpio_name, "aux-en") ||
				...
				gpio_direction_output(config->gpio,
					config->value);
			else if (dp_power_find_gpio(config->gpio_name, "uart-en"))
				{
				 uart_en_init_state=gpio_get_value(config->gpio);
				 DP_INFO("%s: uart init state=%d,disable uart \n",__func__,uart_en_init_state);
	...
	if (enable) {
		...
		for (i = 0; i < mp->num_gpio; i++) {
			if (gpio_is_valid(config[i].gpio))
				{
			      if (dp_power_find_gpio(config[i].gpio_name, "uart-en"))
				 {
                    DP_INFO("%s: uart init state=%d, restore uart to default state \n",__func__,uart_en_init_state);
				    gpio_direction_output(config[i].gpio, uart_en_init_state);
