...
static void dsi_display_parse_te_data(struct dsi_display *display)
{
	...
	display->disp_te1_left_gpio = of_get_named_gpio(dev->of_node,
					"qcom,platform-te1-left-gpio", 0);
	display->disp_te1_right_gpio = of_get_named_gpio(dev->of_node,
					"qcom,platform-te1-right-gpio", 0);
    ...
    if (gpio_is_valid(display->disp_te1_left_gpio)) {
        rc = gpio_request(display->disp_te1_left_gpio, "te1-left-gpio");
        if (rc) {
            DSI_ERR("request for te1-left-gpio failed, rc=%d\n",rc);
        }else{
            rc = gpio_direction_input(display->disp_te1_left_gpio);
            if(rc){
                DSI_ERR("gpio_direction_input for te1-left-gpio failed, rc=%d\n",rc); 
            }
        }
    }

    if (gpio_is_valid(display->disp_te1_right_gpio)) {
        rc = gpio_request(display->disp_te1_right_gpio, "te1-right-gpio");
        if (rc) {
            DSI_ERR("request for te1-right-gpio failed, rc=%d\n",rc);
        }else{
            rc = gpio_direction_input(display->disp_te1_right_gpio);
            if(rc){
                DSI_ERR("gpio_direction_input for te1-right-gpio failed, rc=%d\n",rc);
            }
        }
    }
...
static int dsi_display_status_check_te1_esd(struct dsi_display *display)
{
    int rc = 1;
    int i;
    int te1_gpio_left_level = 0;
    int te1_gpio_right_level = 0;

    for(i=0;i<3;i++){
        if(gpio_is_valid(display->disp_te1_left_gpio)){
            te1_gpio_left_level = gpio_get_value(display->disp_te1_left_gpio);
        }

        if(gpio_is_valid(display->disp_te1_right_gpio)){
            te1_gpio_right_level = gpio_get_value(display->disp_te1_right_gpio);
        }

        if(te1_gpio_left_level || te1_gpio_right_level){
            pr_err("left gpio:%d right gpio:%d\n",te1_gpio_left_level,te1_gpio_right_level);
            rc = -EINVAL;
            break;
        }
					...
					bool te_check_override)
{
	...
	if (status_mode == ESD_MODE_REG_READ) {
		...
		rc = dsi_display_status_check_te(dsi_display);
	} else if (status_mode ==  ESD_MODE_PANEL_ESD_TE1){
        rc = dsi_display_status_check_te1_esd(dsi_display);
    } else {
		...
		const char *buf, size_t count)
{
	...
	if (buf == NULL || (display->panel->panel_initialized == 0))
		return retval;
...
static DEVICE_ATTR(read, S_IWUSR | S_IRUSR, dsi_access_sysfs_read_show,
		dsi_access_sysfs_read_store);
		...
		const char *buf, size_t count)
{
	...
	if (buf == NULL || !display->panel->panel_initialized)
	{
		pr_err("buf is NULL or panel_initialized is %d\n", display->panel->panel_initialized);
		return retval;
	}
...
static DEVICE_ATTR(write, S_IWUSR, NULL, dsi_access_sysfs_write_store);
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if ((mode->priv_info->dsc_enabled) &&
			!(mode->dsi_mode_flags & DSI_MODE_FLAG_DMS_FPS) && pps_disable) {
