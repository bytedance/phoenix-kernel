...
static void dsi_bridge_enable(struct drm_bridge *bridge)
{
    ...
    pr_err("dsi_bridge_enable entered rate:%d\n",cur->timing.refresh_rate);
    if(cur->timing.refresh_rate == 72) {
        rc = dsi_panel_post_enable_72_nt57900(display->panel);
    }
    if (cur->timing.refresh_rate == 90) {
        if(hw_version_display){
            rc = dsi_panel_post_enable_90_nt57900(display->panel);
        }
    }
    if (cur->timing.refresh_rate == 120) {
        rc = dsi_panel_post_enable_120_nt57900(display->panel);
    }
