...
extern int aw3750x_enp_enn_set(int val);
extern int hw_version_get(void);
int hw_version_display;
				...
				enum dsi_cmd_set_type type)
{
    ...
    cmds->msg.tx_flag = 0x0;
    pr_err("panel->dts_panel_type:%d\n",panel->dts_panel_type);

	for (i = 0; i < count; i++) {

        if(panel->dts_panel_type == 2){
            if(type == DSI_CMD_SET_POST_72_NT57900){
                pbuf = (u8 *)cmds->msg.tx_buf;
                if(i==4){                                       // right streen
                    if(panel->vcompara_flag == 0) goto nextcmd;
                    for(j=0;j<6;j++){
                        pbuf[j+1] = panel->vcompara_sharp_mipi1_72hz[j];
                    }
                }
                ...
                if(i==7){
                    if(panel->vcompara_flag == 0) goto nextcmd;
                    for(j=0;j<6;j++){
                        pbuf[j+1] = panel->vcompara_sharp_mipi0_72hz[j];
                    }
                }

                if( (i==5) || (i==6) || (i==7)) cmds->msg.tx_flag = 0x01;
                else cmds->msg.tx_flag = 0x0;
            } else if(type == DSI_CMD_SET_POST_90_NT57900){
                pbuf = (u8 *)cmds->msg.tx_buf;
                if(i==4){                                       // right streen
                    if(panel->vcompara_flag == 0) goto nextcmd;
                    for(j=0;j<6;j++){
                        pbuf[j+1] = panel->vcompara_sharp_mipi1_90hz[j];
                    }
                }
                ...
                if(i==7){
                    if(panel->vcompara_flag == 0) goto nextcmd;
                    for(j=0;j<6;j++){
                        pbuf[j+1] = panel->vcompara_sharp_mipi0_90hz[j];
                    }
                }
...
static int dsi_panel_parse_vcom_para(struct dsi_panel *panel)
{
    char *pVcomPara = NULL;
    int i;
    u8 vcompara_sharp[8];

    panel->vcompara_flag = 0;
    pVcomPara = strstr(boot_command_line, "VcomPara=");

    if(pVcomPara){
        panel->vcompara_flag = 1;

        for(i=0;i<8;i++){
            vcompara_sharp[i] = HexcharToInt(pVcomPara[i*2+9])*16 + HexcharToInt(pVcomPara[i*2+10]);
            if(vcompara_sharp[i] == 0) panel->vcompara_flag = 0;
        }

        for(i=0;i<3;i++){
            panel->vcompara_sharp_mipi0_72hz[i*2] = vcompara_sharp[0];
            panel->vcompara_sharp_mipi0_72hz[i*2+1] = vcompara_sharp[1];
            panel->vcompara_sharp_mipi0_90hz[i*2] = vcompara_sharp[2];
            panel->vcompara_sharp_mipi0_90hz[i*2+1] = vcompara_sharp[3];

            panel->vcompara_sharp_mipi1_72hz[i*2] = vcompara_sharp[4];
            panel->vcompara_sharp_mipi1_72hz[i*2+1] = vcompara_sharp[5];
            panel->vcompara_sharp_mipi1_90hz[i*2] = vcompara_sharp[6];
            panel->vcompara_sharp_mipi1_90hz[i*2+1] = vcompara_sharp[7];
        }

        pr_err("panel->vcompara_sharp:%2x %2x %2x %2x %2x %2x %2x %2x\n",vcompara_sharp[0],vcompara_sharp[1],vcompara_sharp[2],vcompara_sharp[3],vcompara_sharp[4],
        vcompara_sharp[5],vcompara_sharp[6],vcompara_sharp[7]);

    } else {
        pr_err("%s parse VcomPara failed \n",__func__);
        return -EINVAL;
    }
...
static int dsi_panel_parse_dfps_caps(struct dsi_panel *panel)
{
    ...
    hw_version_display = hw_version_get();
    pr_err("hw_version_display:%d \n",hw_version_display);
    if(hw_version_display == 0) supported = false;
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-post-72-nt57900-on-command",
	"qcom,mdss-dsi-post-90-nt57900-on-command",
	"qcom,mdss-dsi-post-120-nt57900-on-command",
	"qcom,mdss-dsi-on-command_p1",
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-on-72-nt57900-command-state",
	"qcom,mdss-dsi-on-90-nt57900-command-state",
	"qcom,mdss-dsi-on-120-nt57900-command-state",
	"qcom,mdss-dsi-on-command-p1-state",
...
static int dsi_panel_parse_esd_config(struct dsi_panel *panel)
{
	...
	if (!rc) {
		...
		} else if (!strcmp(string, "te1_esd_check")) {
            esd_config->status_mode = ESD_MODE_PANEL_ESD_TE1;
        } else {
	...
	if (panel->esd_config.status_mode == ESD_MODE_REG_READ) {
		...
		esd_mode = "te_check";
	} else if(panel->esd_config.status_mode == ESD_MODE_PANEL_ESD_TE1){
        esd_mode = "te1_esd_check";
    }
				...
				int topology_override)
{
    ...
    panel->dts_panel_type = 0;
	if (!panel->name)
		panel->name = DSI_PANEL_DEFAULT_LABEL;
    else if(!strncmp(panel->name,"innolux nt57900",15))
        panel->dts_panel_type = 1;
    else if(!strncmp(panel->name,"sharp ls026b3sa",15))
        panel->dts_panel_type = 2;

    if(panel->dts_panel_type == 2)
        dsi_panel_parse_vcom_para(panel);
	/*
...
int dsi_panel_enable(struct dsi_panel *panel)
{
    ...
    pr_err("dsi_panel_enable DSI_CMD_SET_ON \n");

	mutex_lock(&panel->panel_lock);

    if(hw_version_display == 0){
        rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_ON_P1);
    }
    else {
...
int dsi_panel_post_enable_72_nt57900(struct dsi_panel *panel)
{
  int rc = 0;
  ...
  rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_POST_72_NT57900);
  if (rc) {
      DSI_ERR("[%s] failed to send DSI_CMD_SET_POST_72_NT57900 cmds, rc=%d\n",
             panel->name, rc);
      goto error;
  }
error:
...
int dsi_panel_post_enable_90_nt57900(struct dsi_panel *panel)
{
  int rc = 0;
  ...
  rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_POST_90_NT57900);
  if (rc) {
      DSI_ERR("[%s] failed to send DSI_CMD_SET_POST_90_NT57900 cmds, rc=%d\n",
             panel->name, rc);
      goto error;
  }
error:
...
int dsi_panel_post_enable_120_nt57900(struct dsi_panel *panel)
{
  int rc = 0;
  ...
  rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_POST_120_NT57900);
  if (rc) {
      DSI_ERR("[%s] failed to send DSI_CMD_SET_POST_120_NT57900 cmds, rc=%d\n",
             panel->name, rc);
      goto error;
  }
error:
