...
retry:
	...
	enable = cpumask_test_cpu(7, irq_get_affinity_mask(sde_kms->irq_num));
	if(cpu_online(7) && (sde_kms->irq_num > 0) && !enable){
		irq_set_affinity(sde_kms->irq_num, get_cpu_mask(7));
	} else {
		printk("msm_drm affinity irq =%d enable =%d \n",sde_kms->irq_num, enable);
	}
...
static int sde_kms_hw_init(struct msm_kms *kms)
{
	...
	if(cpu_online(7) && (irq_num > 0)){
		irq_set_affinity(irq_num, get_cpu_mask(7));
	} else {
		printk("msm_drm irq = %d\n", irq_num);
	}
	return 0;
