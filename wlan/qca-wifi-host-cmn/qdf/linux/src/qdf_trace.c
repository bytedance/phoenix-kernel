...
static uint32_t get_str_buffer_index(void)
{
	uint32_t qsb_index = 0;
	unsigned long flags;

	spin_lock_irqsave(&qdf_str_buffer_lock, flags);
	...
	spin_unlock_irqrestore(&qdf_str_buffer_lock, flags);
...
static void release_str_buffer(uint32_t qsb_index)
{
	unsigned long flags;
...
void qdf_trace_msg_cmn(unsigned int idx,
			...
			const char *str_format, va_list val)
{
	char stack_buffer[QDF_TRACE_STACK_BUFFER_SIZE];
	char *str_buffer;
	uint32_t tmpqsb_index = QDF_STR_BUFFER_NUM;
	int16_t n, m;
	...
	if (category < 0 || category >= MAX_SUPPORTED_CATEGORY) {
		vscnprintf(stack_buffer, QDF_TRACE_STACK_BUFFER_SIZE, str_format, val);
		pr_info("%s: Invalid category: %d, log: %s\n",
			__func__, category, stack_buffer);
		return;
	}
	...
	if (print_ctrl_obj[idx].cat_info[category].category_verbose_mask &
		...
		n = scnprintf(stack_buffer, QDF_TRACE_STACK_BUFFER_SIZE,
		...
		m = vscnprintf(stack_buffer + n, QDF_TRACE_STACK_BUFFER_SIZE - n,
			   str_format, val);

		str_buffer = stack_buffer;

		if (m + n >= QDF_TRACE_STACK_BUFFER_SIZE - 1) {
			...
			if (tmpqsb_index < QDF_STR_BUFFER_NUM) {
				...
				scnprintf(str_buffer, n + 1, "%s", stack_buffer);
