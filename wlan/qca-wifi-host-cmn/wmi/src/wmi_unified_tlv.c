				   ...
				   uint32_t service_id)
{
	...
	/* Added by PICO_Driver jim.qiu for bug62291: do not output log such as but WMI ext2 service bitmap is NULL --start-- */
	if(WMI_SERVICE_LL_STATS_PER_CHAN_RX_TX_TIME_SUPPORT == service_id ||
	WMI_SERVICE_UNIFIED_LL_GET_STA_CMD_SUPPORT == service_id ||
	WMI_SERVICE_FSE_CMEM_ALLOC_SUPPORT == service_id ||
	WMI_SERVICE_SCAN_CONFIG_PER_CHANNEL == service_id ||
	WMI_SERVICE_CSA_BEACON_TEMPLATE == service_id ||
	WMI_SERVICE_BROADCAST_TWT_REQUESTER == service_id ||
	WMI_SERVICE_BROADCAST_TWT_RESPONDER == service_id
	){
	        return false;
	}
	/* Added by PICO_Driver jim.qiu for bug62291: do not output log such as but WMI ext2 service bitmap is NULL --end-- */
